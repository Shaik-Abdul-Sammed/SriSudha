...
import { ArrowLeft, Send, Sparkles, DollarSign, PieChart, Download, Copy, Check } from 'lucide-react';
...
export default function FinanceOfficer() {
  ...
  useEffect(() => {
    if (error) {
      console.error('Finance stream error:', error);
  ...
  const handleSend = useCallback(
    (text) => {
      ...
      const endpoint = `${getApiBaseURL()}/v1/officers/finance/stream`;
  ...
  const handleDownloadPdf = async (content) => {
    ...
    if (!textToExport) {
      addToast('No financial report content available to download.', 'warning');
      return;
    }

    try {
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken');
      const response = await fetch(`${getApiBaseURL()}/v1/officers/finance/export-pdf`, {
        method: 'POST',
        headers: {
      ...
      a.href = url;
      a.download = `eduflow-finance-report-${Date.now()}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);

      addToast('Financial PDF report downloaded successfully.', 'success');
    } catch (err) {
      ...
      <div style={{
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {[
              "Reconcile Fee Collections with Bank Statement",
              "Audit Unmatched UPI Transactions",
              "Scan Arrears & Generate Follow-ups",
              "Disburse Approved Scholarships",
            ].map((action, idx) => (
              <button key={idx} onClick={() => handleSend(action)} style={{
                ...
                <DollarSign size={18} color="#ec4899" /> {action}
              </button>
            ))}
          </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '1.5rem', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.875rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><DollarSign size={16} color="#10B981" /> Reconciled</span>
                <span className="font-bold text-emerald-400">₹42,50,000 (88.5%)</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><PieChart size={16} color="#ef4444" /> Unmatched UPI</span>
                <span className="font-bold text-rose-400">3 Flagged (₹38.5k)</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><span style={{ color: '#f59e0b' }}>●</span> Pending Arrears</span>
                <span className="font-bold text-amber-400">₹1,80,000 (14 acc)</span>
              </div>
            </div>
          </div>

          <div style={{
            marginTop: '2rem', background: 'linear-gradient(135deg, rgba(236, 72, 153, 0.2), rgba(6, 182, 212, 0.2))',
            padding: '1rem', borderRadius: '12px', border: '1px solid rgba(236, 72, 153, 0.3)',
            display: 'flex', alignItems: 'center', gap: '1rem',
          }}>
            <Sparkles color="#ec4899" size={24} />
            <div>
              <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Revenue Saved</div>
              <div style={{ fontWeight: 'bold' }}>₹3,200 audit recovery</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', position: 'relative' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.25rem' }}>
            ...
            <h2 style={{ fontSize: '1.2rem', fontWeight: 'bold', margin: 0 }}>AI Finance Officer</h2>
          </div>
          <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(255,255,255,0.6)' }}>
            Automated UPI/NEFT fee reconciliation, bank statement audits, and fee defaulter tracking.
          </p>
        ...
        {messages.length === 0 && status === 'idle' ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
            <div style={{
              ...
              border: '1px solid rgba(236, 72, 153, 0.3)', boxShadow: '0 0 30px rgba(236, 72, 153, 0.2)',
            }}>
              💰
            </div>
            <h2 style={{ fontSize: '1.4rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>Financial Reconciliation Workspace</h2>
            <p style={{ color: 'rgba(255,255,255,0.6)', maxWidth: '480px', textAlign: 'center', marginBottom: '2rem' }}>
              Ask the officer anything about fee reconciliation, UPI transaction matching, or arrears tracking.
            </p>
          </div>
        ) : (
          <div style={{ flex: 1, overflowY: 'auto', padding: '2rem' }}>
            {messages.map((msg, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  background: msg.role === 'user' ? '#2563EB' : 'rgba(236, 72, 153, 0.2)',
                  border: msg.role === 'user' ? 'none' : '1px solid rgba(236, 72, 153, 0.5)',
                }}>
                  {msg.role === 'user' ? 'SA' : '💰'}
                </div>
                <div style={{
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '0.75rem' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          color: '#f472b6', padding: '0.35rem 0.75rem', borderRadius: '6px',
                          fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.35rem', cursor: 'pointer',
                        }}
                      >
            ...
            {status === 'streaming' && (
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexDirection: 'row' }}>
                ...
                <div style={{
                  ...
                  border: '1px solid rgba(236, 72, 153, 0.3)', lineHeight: '1.6',
                }}>
                  {formatMessage(tokens)}
                  <span style={{
                    display: 'inline-block', width: '8px', height: '16px', background: '#ec4899',
            ...
            {status === 'connecting' && (
              <div style={{ color: '#ec4899', fontSize: '0.85rem', fontStyle: 'italic', margin: '0.5rem 0' }}>
                [Finance Officer]: Connecting to bank escrow accounts and reconciling transactions...
              </div>
            )}
        ...
        <div style={{ padding: '1.5rem 2rem', borderTop: '1px solid rgba(255,255,255,0.1)', background: '#0A0E27' }}>
          <div style={{
            ...
            <textarea
              ...
              placeholder="Ask me to reconcile fees, audit unmatched UPI deposits, scan defaulters..."
              style={{
            ...
            <button
              ...
              style={{
                background: inputValue.trim() && status !== 'streaming' ? '#ec4899' : 'rgba(255,255,255,0.1)',
