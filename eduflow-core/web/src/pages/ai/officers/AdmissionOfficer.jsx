...
import { ArrowLeft, Send, Sparkles, TrendingUp, Users, Download, Copy, Check } from 'lucide-react';
...
export default function AdmissionOfficer() {
  ...
  useEffect(() => {
    if (error) {
      console.error('Admissions stream error:', error);
  ...
  const handleSend = useCallback(
    (text) => {
      ...
      const endpoint = `${getApiBaseURL()}/v1/officers/admissions/stream`;
  ...
  const handleDownloadPdf = async (content) => {
    ...
    try {
      ...
      const response = await fetch(`${getApiBaseURL()}/v1/officers/admissions/export-pdf`, {
        method: 'POST',
        headers: {
      ...
      a.href = url;
      a.download = `eduflow-admissions-report-${Date.now()}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);

      addToast('Admissions PDF downloaded successfully.', 'success');
    } catch (err) {
      ...
      <div style={{
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {[
              "Predict Application Conversion Yield",
              "Audit Drop-off Bottlenecks",
              "Queue Automated Offer Letters",
              ...
              <button key={idx} onClick={() => handleSend(action)} style={{
                ...
                <TrendingUp size={18} color="#f59e0b" /> {action}
              </button>
            ))}
          </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '1.5rem', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.875rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Users size={16} color="#38bdf8" /> Applications</span>
                <span className="font-bold">450 Evaluated</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><TrendingUp size={16} color="#10B981" /> Predicted Yield</span>
                <span className="font-bold text-emerald-400">72.4% (275)</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><span style={{ color: '#f59e0b' }}>●</span> Waitlist Pool</span>
                <span className="font-bold text-amber-400">65 Candidates</span>
              </div>
            </div>
          </div>

          <div style={{
            marginTop: '2rem', background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.2), rgba(6, 182, 212, 0.2))',
            padding: '1rem', borderRadius: '12px', border: '1px solid rgba(245, 158, 11, 0.3)',
            display: 'flex', alignItems: 'center', gap: '1rem',
          }}>
            <Sparkles color="#f59e0b" size={24} />
            <div>
              <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Yield Impact</div>
              <div style={{ fontWeight: 'bold' }}>+18% conversion velocity</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', position: 'relative' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.25rem' }}>
            ...
            <h2 style={{ fontSize: '1.2rem', fontWeight: 'bold', margin: 0 }}>AI Admission Officer</h2>
          </div>
          <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(255,255,255,0.6)' }}>
            Predictive enrollment yield modeling, applicant concierge, and automated counseling conversion.
          </p>
        ...
        {messages.length === 0 && status === 'idle' ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
            ...
            <h2 style={{ fontSize: '1.4rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>Admissions Intelligence Workspace</h2>
            <p style={{ color: 'rgba(255,255,255,0.6)', maxWidth: '480px', textAlign: 'center', marginBottom: '2rem' }}>
              Ask the officer anything about enrollment yield, applicant conversion, or counseling funnels.
            </p>
          </div>
        ) : (
          <div style={{ flex: 1, overflowY: 'auto', padding: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '0.75rem' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          color: '#fcd34d', padding: '0.35rem 0.75rem', borderRadius: '6px',
                          fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.35rem', cursor: 'pointer',
                        }}
                      >
            ...
            {status === 'streaming' && (
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexDirection: 'row' }}>
                ...
                <div style={{
                  ...
                  border: '1px solid rgba(245, 158, 11, 0.3)', lineHeight: '1.6',
                }}>
                  {formatMessage(tokens)}
                  <span style={{
                    display: 'inline-block', width: '8px', height: '16px', background: '#f59e0b',
            ...
            {status === 'connecting' && (
              <div style={{ color: '#f59e0b', fontSize: '0.85rem', fontStyle: 'italic', margin: '0.5rem 0' }}>
                [Admissions Officer]: Evaluating conversion funnel parameters & yield distributions...
              </div>
            )}
        ...
        <div style={{ padding: '1.5rem 2rem', borderTop: '1px solid rgba(255,255,255,0.1)', background: '#0A0E27' }}>
          <div style={{
            ...
            <textarea
              ...
              placeholder="Ask me to predict yield, analyze drop-off bottlenecks, generate follow-up plans..."
              style={{
            ...
            <button
              ...
              style={{
                background: inputValue.trim() && status !== 'streaming' ? '#f59e0b' : 'rgba(255,255,255,0.1)',
